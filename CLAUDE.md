...
Centralized user options live in `modules/user/`. These expose settings like `username`, `hostName`, `browser`, `colorScheme`, `wallpaper`, and `dots` that other modules reference via `config.userOptions.*`.

### Theming

All theming is handled by **Stylix** (currently Gruvbox Dark Hard, set per-host via `userOptions.colorScheme`). Don't hardcode colors or fonts — reference Stylix values so the entire system theme changes from one place.
...
- Display: Wayland / Hyprland + Noctalia
- Terminal: Ghostty
- Shell: Zsh + Powerlevel10k (fish + starship available as a dormant alternative)
- Editor: Neovim (nix-wrapped), Zed
- Browser: Brave (Desktop) / Helium (Laptop); LibreWolf available via the firefox module
